    ...
    <div className="rounded-lg border border-line bg-panel">
      ...
      <div className="max-h-80 overflow-y-auto font-mono text-xs">
        {entries.map((e) => (
          <div
            key={e.id}
            className="flex items-center gap-4 px-5 py-2.5 border-b border-line/60 last:border-0"
          >
            <span className="text-muted w-20 shrink-0">{e.time}</span>
            <span className="text-wire w-14 shrink-0">{e.id}</span>
            <span className="text-text flex-1 truncate">{e.type}</span>
